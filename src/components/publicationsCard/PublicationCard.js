  ...
  return (
    ...
    >
      <Fade bottom duration={2000} distance="40px">
        <div key={repo.id} onClick={() => openRepoinNewTab(repo.createdAt)}>
          <div className="publication-name-div">
            <p className="publication-name" style={{ color: theme.text }} onClick={() => openRepoinNewTab(repo.createdAt)}>
              {repo.name}
            </p>
          </div>
          <p className="publication-description" style={readMore?{ color: theme.text}:{ color: theme.texts,WebkitLineClamp:2}}>
            {repo.description}
          </p>
          <a onClick={(e)=>{
            e.preventDefault();
